...
public class EventService {
    ...
    public List<EventFullDto> findAllByAdmin(List<Long> users,
        ...
        if (rangeEnd == null) {
            rangeEnd = Constants.getMaxDateTime();
        }
        ...
        List<ViewStatsDto> viewStatsDtoList = statsClient.getStats(rangeStart.format(Constants.getDefaultDateTimeFormatter()),
                rangeEnd.format(Constants.getDefaultDateTimeFormatter()), eventUrls, true);

        return page.getContent().stream()
                .map(EventMapper.INSTANCE::toFullDto)
                .peek(dto -> {
                    Optional<ViewStatsDto> matchingStats = viewStatsDtoList.stream()
    ...
    @Transactional(readOnly = true)
    public List<EventShortDto> findAllByInitiator(long userId, int from, int size) {
        Pageable pageable = PageRequest.of(from, size);
    ...
    @Transactional(readOnly = true)
    public EventFullDto findByIdByInitiator(long userId, long eventId) {
        Event event = findEventByIdIfExists(eventId);
        checkInitiatorConsistency(userId, eventId, event.getInitiator().getId());

        return EventMapper.INSTANCE.toFullDto(event);
    }

    @Transactional(readOnly = true)
    public List<ParticipationRequestDto> findParticipationRequestsByInitiator(long userId, long eventId) {
    ...
    @Transactional(readOnly = true)
    public List<EventShortDto> findAllPublic(String text,
                                             List<Long> categories,
                                             Boolean paid,
                                             LocalDateTime rangeStart,
                                             LocalDateTime rangeEnd,
                                             boolean onlyAvailable,
                                             EventControllerPublic.SortType sort,
                                             int from,
                                             int size,
    ...
    public EventFullDto getByIdPublic(long eventId, HttpServletRequest request) {
        ...
        List<ViewStatsDto> viewStatsDtos = statsClient.getStats(Constants.getMinDateTime().format(Constants.getDefaultDateTimeFormatter()),
    ...
    public EventFullDto create(long userId, EventNewDto eventNewDto) {
        ...
        Category category = findCategoryByIdIfExists(eventNewDto.getCategory());
    ...
    @Transactional
    public EventFullDto updateByAdmin(long eventId, EventUpdateAdminRequest updateEventAdminRequest) {
        Event event = findEventByIdIfExists(eventId);
        ...
        if (updateEventAdminRequest.getCategory() != null) {
            event.setCategory(findCategoryByIdIfExists(updateEventAdminRequest.getCategory()));
        }
    ...
    @Transactional
    public EventFullDto updateByInitiator(long userId, long eventId, EventUpdateUserRequest updateEventUserRequest) {
        Event event = findEventByIdIfExists(eventId);
        checkInitiatorConsistency(userId, eventId, event.getInitiator().getId());
        ...
        if (updateEventUserRequest.getCategory() != null) {
            event.setCategory(findCategoryByIdIfExists(updateEventUserRequest.getCategory()));
        }
    ...
    private Category findCategoryByIdIfExists(long id) {
        return categoryRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Category with id=" + id + " not found"));
    }

    private void checkInitiatorConsistency(long userId, long eventId, long initiatorId) {
